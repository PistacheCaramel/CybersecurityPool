...
size_t writeCallback(char *ptr, size_t size, size_t nmemb, char **userdata) {
    size_t total_size = size * nmemb;
    *userdata = (char *)calloc(1, total_size + 1);
    *userdata = strncpy(*userdata, ptr, total_size);
    return total_size;
}
...
void		get_image_data_from_url(const char *url)
{
...
char	*get_data_from_url(const char *url)
{   
    // libcurl Initialize
	std::cout << "Url:" << url << std::endl;
    CURL* curl = curl_easy_init();
